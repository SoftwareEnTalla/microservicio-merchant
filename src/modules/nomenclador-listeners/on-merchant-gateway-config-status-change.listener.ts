 ...
 * Licencia: MIT
 *
 * OnMerchantGatewayConfigStatusChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `merchant-gateway-config-status` (owner: merchant). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'merchant-gateway-config-status';
const OWNER_BC = 'merchant';
...
const TOPICS: string[] = [
  'merchant-gateway-config-status-created',
  'merchant-gateway-config-status-updated',
  'merchant-gateway-config-status-deleted',
];

@Injectable()
export class OnMerchantGatewayConfigStatusChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnMerchantGatewayConfigStatusChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnMerchantGatewayConfigStatusChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnMerchantGatewayConfigStatusChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onMerchantGatewayConfigStatusChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
