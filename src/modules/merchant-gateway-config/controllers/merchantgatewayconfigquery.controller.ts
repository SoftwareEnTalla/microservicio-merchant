...
@ApiTags("MerchantGatewayConfig Query")
@Controller("merchantgatewayconfigs/query")
export class MerchantGatewayConfigQueryController {
  #logger = new Logger(MerchantGatewayConfigQueryController.name);

  constructor(private readonly service: MerchantGatewayConfigQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all merchantgatewayconfig with optional pagination" })
  @ApiResponse({ status: 200, type: MerchantGatewayConfigsResponse })
  @ApiQuery({ name: "options", required: false, type: MerchantGatewayConfigDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<MerchantGatewayConfig>    
  ): Promise<MerchantGatewayConfigsResponse<MerchantGatewayConfig>> {
    try {
     
      const merchantgatewayconfigs = await this.service.findAll(options);
      logger.info("Retrieving all merchantgatewayconfig");
  ...
  @ApiOperation({ summary: "Find merchantgatewayconfig by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter merchantgatewayconfig', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: MerchantGatewayConfigsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "MerchantGatewayConfig no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find merchantgatewayconfigs with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<MerchantGatewayConfig>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all merchantgatewayconfigs" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count merchantgatewayconfigs with conditions" })
  @ApiResponse({ status: 200, type: MerchantGatewayConfigsResponse<MerchantGatewayConfig> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one merchantgatewayconfig with conditions" })
  ...
  @ApiOperation({ summary: "Find one merchantgatewayconfig or return error" })
  @ApiResponse({ status: 200, type: MerchantGatewayConfigResponse<MerchantGatewayConfig> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get merchantgatewayconfig by ID" })
  @ApiResponse({ status: 200, type: MerchantGatewayConfigResponse<MerchantGatewayConfig> })
  @ApiResponse({ status: 404, description: "MerchantGatewayConfig not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the merchantgatewayconfig to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<MerchantGatewayConfigResponse<MerchantGatewayConfig>> {
    try {
      const merchantgatewayconfig = await this.service.findOne({ where: { id } });
      if (!merchantgatewayconfig) {
        throw new NotFoundException(
          "MerchantGatewayConfig no encontrado para el id solicitado"
        );
      }
      return merchantgatewayconfig;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }
