...
import { MerchantDto } from "../dtos/all-dto";
...
@ApiTags("Merchant Query")
@Controller("merchants/query")
export class MerchantQueryController {
  #logger = new Logger(MerchantQueryController.name);
  ...
  @Get("list")
  @ApiOperation({ summary: "Get all merchant with optional pagination" })
  @ApiResponse({ status: 200, type: MerchantsResponse })
  @ApiQuery({ name: "options", required: false, type: MerchantDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Merchant>    
  ): Promise<MerchantsResponse<Merchant>> {
    try {
     
      const merchants = await this.service.findAll(options);
      logger.info("Retrieving all merchant");
      return merchants;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find merchant by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter merchant', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: MerchantsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Merchant no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find merchants with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Merchant>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all merchants" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count merchants with conditions" })
  @ApiResponse({ status: 200, type: MerchantsResponse<Merchant> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one merchant with conditions" })
  ...
  @ApiOperation({ summary: "Find one merchant or return error" })
  @ApiResponse({ status: 200, type: MerchantResponse<Merchant> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  /**
   * Búsqueda semántica / textual sobre merchants.
   *
   * - `semanticSearch=true` (default) usa cosine similarity sobre `semanticEmbedding`.
   * - `semanticSearch=false` fuerza búsqueda textual.
   * - Si el modo semántico no produce resultados sobre el umbral, cae a búsqueda
   *   textual devolviendo `searchMode=TEXTUAL_FALLBACK`.
   *
   * IMPORTANTE: declarado ANTES de `@Get(":id")` para evitar que `:id` capture
   * el segmento `semantic-search`.
   */
  @Get("semantic-search")
  @ApiOperation({ summary: "Semantic (pgvector) or textual search over merchants" })
  ...
  ): Promise<{ ok: boolean; data: Merchant[]; totalCount: number; searchMode: string; similarityThreshold: number; scores?: number[] }> {
    try {
      ...
      const items: Merchant[] = Array.isArray(pageResp?.data) ? pageResp.data : (Array.isArray(pageResp?.items) ? pageResp.items : []);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the merchant to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<MerchantResponse<Merchant>> {
    try {
      const merchant = await this.service.findOne({ where: { id } });
      if (!merchant) {
        throw new NotFoundException(
