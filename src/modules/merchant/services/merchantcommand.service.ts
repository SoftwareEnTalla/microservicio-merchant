...
import { CreateMerchantDto, UpdateMerchantDto, DeleteMerchantDto } from "../dtos/all-dto";
...
export class MerchantCommandService implements OnModuleInit {
  // Private properties
  readonly #logger = new Logger(MerchantCommandService.name);
  //Constructo del servicio MerchantCommandService
  constructor(
    private readonly repository: MerchantCommandRepository,
    private readonly queryRepository: MerchantQueryRepository,
  ...
  private async publishDslDomainEvents(events: BaseEvent[]): Promise<void> {
    for (const event of events) {
      ...
      if (process.env.EVENT_STORE_ENABLED === "true") {
        await this.eventStore.appendEvent('merchant-' + event.aggregateId, event);
      }
    }
  }
  ...
  private async applyDslServiceRules(
    ...
    if (operation === 'create') {
      // Regla de servicio: merchant-must-reference-user
  ...
  @LogExecutionTime({
    ...
    client: LoggerClient.getInstance()
      .registerClient(MerchantCommandService.name)
      .get(MerchantCommandService.name),
  })
  @Cacheable({
    key: (args) =>
      generateCacheKey<CreateMerchantDto>("createMerchant", args[0], args[1]),
    ttl: 60,
  })
  async create(
    createMerchantDtoInput: CreateMerchantDto
  ): Promise<MerchantResponse<Merchant>> {
    try {
      logger.info("Receiving in service:", createMerchantDtoInput);
      const candidate = Merchant.fromDto(createMerchantDtoInput);
  ...
  @Cacheable({
    key: (args) =>
      generateCacheKey<Merchant>("createMerchants", args[0], args[1]),
    ttl: 60,
  })
  async bulkCreate(
    createMerchantDtosInput: CreateMerchantDto[]
  ): Promise<MerchantsResponse<Merchant>> {
    try {
      const entities = await this.repository.bulkCreate(
        createMerchantDtosInput.map((entity) => Merchant.fromDto(entity))
      );
      ...
      return {
        ok: true,
        message: "Merchants creados con éxito.",
        data: entities,
  ...
  @Cacheable({
    key: (args) =>
      generateCacheKey<UpdateMerchantDto>("updateMerchant", args[0], args[1]),
    ttl: 60,
  })
  async update(
    id: string,
    partialEntity: UpdateMerchantDto
  ): Promise<MerchantResponse<Merchant>> {
    try {
      const currentEntity = await this.queryRepository.findById(id);
      const candidate = Object.assign(new Merchant(), currentEntity ?? {}, partialEntity);
      ...
      return {
        ok: true,
        message: "Merchant actualizada con éxito.",
        data: entity,
      };
    } catch (error) {
      // Imprimir error
  ...
  @Cacheable({
    key: (args) =>
      generateCacheKey<UpdateMerchantDto>("updateMerchants", args[0]),
    ttl: 60,
  })
  async bulkUpdate(
    partialEntity: UpdateMerchantDto[]
    ...
    try {
      const entities = await this.repository.bulkUpdate(
        partialEntity.map((entity) => Merchant.fromDto(entity))
      );
      // Respuesta si el merchant no existe
      if (!entities)
        throw new NotFoundException("Entidades Merchants no encontradas.");
      ...
      return {
        ok: true,
        message: "Merchants actualizadas con éxito.",
        data: entities,
        count: entities.length,
      };
    } catch (error) {
  ...
  @Cacheable({
    key: (args) =>
      generateCacheKey<DeleteMerchantDto>("deleteMerchant", args[0], args[1]),
    ttl: 60,
  })
  async delete(id: string): Promise<MerchantResponse<Merchant>> {
    try {
      const entity = await this.queryRepository.findById(id);
      // Respuesta si el merchant no existe
      if (!entity)
        throw new NotFoundException("Instancias de Merchant no encontradas.");
      ...
      return {
        ok: true,
        message: "Instancia de Merchant eliminada con éxito.",
        data: entity,
      };
    } catch (error) {
      // Imprimir error
  ...
  @Cacheable({
    key: (args) => generateCacheKey<string[]>("deleteMerchants", args[0]),
    ttl: 60,
  })
