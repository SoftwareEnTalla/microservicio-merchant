...
import { MerchantApprovalStatusDto } from "../dtos/all-dto";
...
@ApiTags("MerchantApprovalStatus Query")
@UseGuards(MerchantApprovalStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("merchantapprovalstatuss/query")
export class MerchantApprovalStatusQueryController {
  #logger = new Logger(MerchantApprovalStatusQueryController.name);

  constructor(private readonly service: MerchantApprovalStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all merchantapprovalstatus with optional pagination" })
  @ApiResponse({ status: 200, type: MerchantApprovalStatussResponse })
  @ApiQuery({ name: "options", required: false, type: MerchantApprovalStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<MerchantApprovalStatus>    
  ): Promise<MerchantApprovalStatussResponse<MerchantApprovalStatus>> {
    try {
     
      const merchantapprovalstatuss = await this.service.findAll(options);
      logger.info("Retrieving all merchantapprovalstatus");
  ...
  @ApiOperation({ summary: "Find merchantapprovalstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter merchantapprovalstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: MerchantApprovalStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "MerchantApprovalStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find merchantapprovalstatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<MerchantApprovalStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all merchantapprovalstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count merchantapprovalstatuss with conditions" })
  @ApiResponse({ status: 200, type: MerchantApprovalStatussResponse<MerchantApprovalStatus> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one merchantapprovalstatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one merchantapprovalstatus or return error" })
  @ApiResponse({ status: 200, type: MerchantApprovalStatusResponse<MerchantApprovalStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get merchantapprovalstatus by ID" })
  @ApiResponse({ status: 200, type: MerchantApprovalStatusResponse<MerchantApprovalStatus> })
  @ApiResponse({ status: 404, description: "MerchantApprovalStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the merchantapprovalstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<MerchantApprovalStatusResponse<MerchantApprovalStatus>> {
    try {
      const merchantapprovalstatus = await this.service.findOne({ where: { id } });
